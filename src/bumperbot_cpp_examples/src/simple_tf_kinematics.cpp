...

{
    ...
    last_orientation_.setRPY(0, 0, 0); //converts the euler to quanternion
...
void SimpleTfKinematics::timerCallback(){
    ...
    dynamic_transform_stamped_.transform.translation.x = x_increment_+last_x_;
    ...
    dynamic_tf_broadcaster_->sendTransform(dynamic_transform_stamped_); //inserting the data

    last_x_ = dynamic_transform_stamped_.transform.translation.x; //updating the x position
...
//defining the behaviour of gettransformcallback
                                                ...
                                                const std::shared_ptr<bumperbot_msgs::srv::GetTransform::Response> res)
{
    ...
    try{
        requested_transform =  tf_buffer_->lookupTransform(req->frame_id, req->child_frame_id, tf2::TimePointZero);//obtain the transform published between tf and tf static

    }
    //if exception occures print an error
