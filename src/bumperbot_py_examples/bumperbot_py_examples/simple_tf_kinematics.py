...
class simpleTfKinematics(Node):
    def __init__(self):
        super().__init__("simple_tf_kinematics") #initializes the node with the name

        self.static_tf_broadcaster_ = StaticTransformBroadcaster(self) #creating a broadcaster to send static transforms
        ...
        self.x_increment_ = 0.05 #everytime we execute the timercallback function the base will move 0.05 cm
        self.last_x_ = 0.0
        self.rotations_counter_ = 0
        self.last_orientation_ = quaternion_from_euler(0, 0, 0)
        self.orinetation_increment_ = quaternion_from_euler(0, 0, 0.05)

        self.tf_buffer_ = Buffer()
        self.tf_listener_ = TransformListener(self.tf_buffer_, self)

        self.static_transform_stamped_.header.stamp = self.get_clock().now().to_msg()  #accessing the time which the transorm published
        ...
        #zero roatation between matrixes
        ...
        self.static_tf_broadcaster_.sendTransform(self.static_transform_stamped_)#sends the static transform to the tf2
    ...
    def getTransformCallback(self, req, res):
        ...
        try:
            requested_transform = self.tf_buffer_.lookup_transform(req.frame_id, req.child_frame_id, rclpy.time.Time()) #the function allows to use the tf library in order to know the current transform matrix between any two frames
    ...
    def timerCallback(self):
        ...
        self.dynamic_transform_stamped_.transform.translation.z = 0.0
                #zero roatation between matrixes
        q=quaternion_multiply(self.last_orientation_, self.orinetation_increment_)
        ...
        if self.rotations_counter_ >=100:
            self.orinetation_increment_ = quaternion_inverse(self.orinetation_increment_)
            self.rotations_counter_ = 0
