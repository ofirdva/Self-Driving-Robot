...
class SimpleServiceClient (Node):
    def __init__(self , a ,b):
        super().__init__("simple_service_client")#name of the Node

        self.client_ = self.create_client(AddTwoInts, "add_two_ints") 

        while not self.client_.wait_for_service(timeout_sec=1.0):
            self.get_logger().info("Service not availble, waiting again..")
        ...
        self.future_ = self.client_.call_async(self.req_) #send the request message to the service srver
